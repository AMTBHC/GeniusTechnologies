...
function runPageScripts(page) {
  try {
    ...
    console.error(
      `Error al ejecutar los scripts de la página ${page}: `,
      error
    );
  }
}
...
function visible() {
  const sections = document.querySelectorAll(
    ".services, .about, .testimonials, .contact, .cta-section, .metrics, .redes, .hero, .about-section"
  );
...
function carrusel() {
  ...
  if (carousel && nextButton && prevButton) {
    ...
    console.error(
      "No se encontraron los elementos del carrusel o los botones."
    );
  }
}
...
function navbarabout() {
  ...
  aboutNavLinks.forEach((link) => {
    link.addEventListener("click", function (event) {
      ...
      if (section) {
        const headerHeight =
          document.querySelector("header")?.offsetHeight || 0; // Altura del header si existe
        window.scrollTo({
          top: section.offsetTop - headerHeight,
          behavior: "smooth",
        });
